...
void setup() {
  ...
  for (int i = 0; i < cols; i++) {
    for (int j = 0; j < rows; j++) {
      ...
      //i*i*j*j
      grid[i][j] = new Cell(i*40,j*40,40,40,49,240,245, i+j);
      //i+j for diagonal
...
void oscEvent(OscMessage theOscMessage) {
  ...
  if (theOscMessage.checkAddrPattern("/foo/playmode")==true) {
...
class Cell {
  ...
  void highlight() {
    stroke(0);
    strokeWeight(3);
    rect(initialX+x,initialY+y,w,h); 
  }
  ...
  void display2(float zoomAmount) {
    ...
    fill(oscR, oscG ,oscB);
    rect(initialX + ((x/40))*(800/zoomAmount),initialY + ((y/40))*(800/zoomAmount),800/zoomAmount,800/zoomAmount); 
  }
